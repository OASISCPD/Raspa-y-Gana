...
import img from '../../assets/imgs/logo+18.png'
                ...
                <div x-show="showModal" className={`fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50 transition-opacity duration-300 ${showModal ? 'opacity-100' : 'opacity-0'}`}>
                    ...
                    <div className="bg-white rounded-2xl p-8 w-[20rem] sm:w-[24rem] py-8 shadow-2xl transform transition-all duration-300">
                        ...
                        <div className='grid grid-cols-2 gap-4 items-center'>
                            <button onClick={onClose} className='px-[2.5rem] sm:px-[4rem] mx-auto py-1 rounded-full min-w-[6rem] bg-gradient-to-r from-zinc-50 to-black duration-300 sm:text-xl'>No!</button>
                            <button onClick={onCloseOk} className='px-[2.5rem] sm:px-[4rem]  mx-auto py-1 rounded-full bg-gradient-to-r from-redMain to-roseMain duration-300 sm:text-xl'>Si!</button>
                        </div>
                    </div>
                </div>
            </div>
