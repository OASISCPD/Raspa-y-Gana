...
export function TemplateMailDesktop() {
    ...
    return (
        <div ref={firstDivRegex} className=" lg:py-[2rem] 2xl:py-[4rem] sm:mx-[2rem] lg:mx-[8rem] xl:mx-[12rem] 2xl:mx-[20rem] textGothamMedium">
            ...
            <h1 className="text-2xl mt-[4rem] xl:mt-[2rem] text-white text-center font-bold sm:text-4xl lg:text-xl xl:text-2xl 2xl:text-4xl">INGRESÁ UNA DIRECCIÓN DE CORREO VÁLIDA <br />
                PARA QUE TE ENVIEMOS UN MAIL CON TU PREMIO</h1>
            <h2 className="text-gray-200 mb-8 mt-4 sm:my-20 lg:my-8  lg:mx-[2rem] font-semibold sm:text-2xl lg:text-sm xl:text-base 2xl:text-xl">
            ...
            <div className="relative flex my-12 lg:my-4 items-center lg:mx-[4rem] 2xl:my-[6rem] 2xl:mx-[4rem]">
                ...
                <input
                    ...
                    className="pl-[3rem] pr-4 py-4  border-gray-300 text-black xl:text-xl font-semibold placeholder-gray-500 h-full w-full border rounded-full focus:outline-none focus:border-indigo-500 "
                    placeholder="M A I L"
                />
            </div>
            <div className="mx-[5rem]">
                <div className="flex items-center mb-4 sm:mb-12">
                    ...
                    />
                    <label onClick={() => window.open('/terms')} className="ms-2 text-sm font-medium text-blue-600   underline sm:text-xl lg:text-base xl:text-lg cursor-pointer hover:underline-offset-2 hover:text-blueMain">Acepto términos y condiciones</label>
                </div>
                <button onClick={sendData} disabled={!buttonActivated} className={`${buttonActivated ? 'bg-gradient-to-r from-greenDark to-greenMain' : 'bg-green-700 bg-opacity- text-opacity-50 '} uppercase rounded-3xl p-2 px-[3rem] cursor-pointer font-semibold text-white sm:text-xl lg:text-sm xl:text-base`}>Enviar</button>
            </div>
            <div className="mt-16 lg:my-8 flex justify-center 2xl:my-[4rem]">
                <img src={logo} className="animate-spin-once-logo w-[16rem] sm:w-[8rem] xl:w-[10rem]" alt="logo empresarial" />
            </div>
