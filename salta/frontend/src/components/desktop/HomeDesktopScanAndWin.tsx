...
export function HomeDesktopScanAndWin() {
    ...
    const [backgroundImage, setBackgroundImage] = useState(fondoRaspaJuegoDesktopScratch);
    ...
    useEffect(() => {
        ...
        Promise.all([loadImage(imgSecond), loadImage(imgRaspado), loadImage(fondoRaspaJuegoDesktopScratch)]).then(() => {
            ...
            {!imagesLoaded || !minimumTimeElapsed || templateLoading ? (
                ...
                >
                    <div className="  mx-auto textGothamMedium relative">
                        {showScratch ? (
                            <div ref={sectionRef} className='px-[1rem] sm:px-[3rem] lg:py-[2rem]'>
                                <TemplateMailDesktop />
                            </div>
                        ) : (
                            <div className='px-[1rem] py-[2rem]'>
                                <div className="flex flex-col text-center w-full mb-4 sm:mb-0 ">
                                    <div className="text-center">
                                        <img
                                            src={img}
                                            className="w-[20rem] sm:w-[30rem] lg:w-[26rem] mx-auto animate-spin-once"
                                            alt="Titulo Alterno"
                                        />
                                    </div>
                                    <div className='py-[0rem]'>
                                        <img src={imgSubTitle} className="w-[16rem] sm:w-[20rem] lg:w-[16rem] mx-auto" alt="" />
                                    </div>
